...
'client' was created that invokes the same methods that are used in the adminInterface.py file The methods needed to 
be altered to take in parameters as in the there original state the values were input into the methods through the 
use of .gets() and so on Furthermore for the methods to be suitable for testing, instead of displaying the result of 
the method execution to the gui it now returns the value This is needed for unit testing for using statements such as 
...
default value may be returned or in the case of get_settings() a string saying 'Nothing was set' was returned When 
creating these tests the partitions were outlined first to help get a sense of what tests needed to be executed Then 
the inputs were organized into various ranges of inputs into eq classes and one value was picked per class In the 
case where a test case failed the method was then updated in order for the test to pass This led to a good bit of 
test driven style development. """
